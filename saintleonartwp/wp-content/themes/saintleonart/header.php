...
<body>
    <header class="header">
        ...
        <nav class="main-nav">
            <h2 class="main-nav__heading2 heading2 hidden" aria-level="2" role="heading">Navigation principale</h2>

            <input class="burger-check" id="burger-check" type="checkbox">
            <label for="burger-check" class="burger"><span class="hidden">&nbsp;</span></label>

            <div class="nav">
                <a class="logo-txt" href="<?php echo esc_url( home_url( '/' ) ); ?>" title="Recharger la page d'accueil">
                ...
                <ul class="main-nav__list">
                    <?php foreach (dw_get_nav_items('header-menu') as $item): ?>
                        <li class="main-nav__list--elt">
                            <?php if(!$item->children): ?>
                            <a href="<?php echo $item->link; ?>" title="Aller vers la page <?php echo $item->label; ?>" class="main-nav__list--link <?php echo $item -> isCurrent ? ' active' : ''; ?>"><?php echo $item->label; ?></a>
                            <?php else: ?>
                            <p class="main-nav__list--submenu submenu"><?php echo $item->label; ?></p>
